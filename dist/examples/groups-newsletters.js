...
async function main() {
    ...
    sock.onMessage(async (ctx) => {
        ...
        if (ctx.text === '!create-channel') {
            const channel = await sock.newsletters.create('CHAMA Updates Channel', 'Official updates and releases from CHAMA Dev');
            await ctx.reply(`📢 *Channel Created!*\n- Name: ${channel.name}\n- JID: ${channel.id}`);
        }
    });
}
main().catch(console.error);
//# sourceMappingURL=groups-newsletters.js.map